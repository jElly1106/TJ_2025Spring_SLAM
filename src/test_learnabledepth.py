...
from assets.new_sequence_folders import NewSequenceFolder
...
parser.add_argument('--data', default='./dataset', type=str, metavar='DIR',
...
parser.add_argument('--mindepth', type=float, default=0.1)  # 减小最小深度
parser.add_argument('--maxdepth', type=float, default=20.0) # 增大最大深度
...
def main():
    ...
    for i, name in enumerate(error_names):
        print('  * {}: {:.4f}'.format(name, errors_depth[i]))
    print('-' * 50)
    ...
    with torch.no_grad():
        ...
        for i, (tgt_img, ref_imgs, poses, intrinsics, tgt_depth,ref_depths) in enumerate(val_loader):
            ...
            if i % int(0.5*args.print_freq) == 0:
                print(' TEST: Abs Error {:.4f}, RMSE {:.4f}, RMSE log {:.4f}'.format(\
                    errors_depth.avg[1], errors_depth.avg[6], errors_depth.avg[7]))
