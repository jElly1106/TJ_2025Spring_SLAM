...
class SequenceFolder(torch.utils.data.Dataset):
    ...
    def __init__(self, root, seed=None, ttype='test.txt', sequence_length=2, sequence_gap = 20, transform=None, height = 240, width = 320):
        ...
        fold_root = '' 
        scenes = [self.root/fold_root/folder.strip() for folder in open(scene_list_path)]
        self.ttype = ttype
    ...
    def crawl_folders(self, sequence_length,sequence_gap):
        ...
        for scene in self.scenes:
            ...
            poses = [np.loadtxt(os.path.join(scene, "pose", "%06d.txt" % frame)) for frame in frames]

            scene_name = os.path.basename(scene)
            intrinsics_matrix = np.loadtxt(os.path.join(scene, "intrinsics_color.txt"))
            # 提取3x3内参矩阵（前3行前3列）
            intrinsics = intrinsics_matrix[:3, :3].astype(np.float32)
            ...
            for i in range(len(frames)):
                frame_id = frames[i]
                img = os.path.join(scene, "color", "%06d.jpg" % frame_id)
                depth = os.path.join(scene, "depth", "%06d.png" % frame_id)
                ...
                for j in shifts:
                    ...
                    frame_id = frames[j]  # 获取实际的帧ID
                    sample['ref_imgs'].append(os.path.join(scene, "color", "%06d.jpg" % frame_id))
                    sample['ref_depths'].append(os.path.join(scene, "depth", "%06d.png" % frame_id))
